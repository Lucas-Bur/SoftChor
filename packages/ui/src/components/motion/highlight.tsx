...
function MotionHighlight<T extends string>({ ref, ...props }: MotionHighlightProps<T>) {
  ...
  const boundsOffsetRef = React.useRef((props as ParentModeMotionHighlightProps)?.boundsOffset)
  boundsOffsetRef.current = (props as ParentModeMotionHighlightProps)?.boundsOffset
  ...
  const render = (childrenToRender: React.ReactNode) => {
    if (mode === 'parent') {
      return (
        <div
          ref={localRef}
